  ...
  render() {
    ...
    return (
      <div>
        Search Item
        {this.state.task && (
          <MDBBtn onClick={this.taskToggle2}>All Task</MDBBtn>
        )}
        {this.state.task && (
          <MDBBtn onClick={this.taskToggle}>Pending Task</MDBBtn>
        )}
        {this.state.task && (
          <MDBBtn onClick={this.taskToggle1}>Completed Task</MDBBtn>
        )}
        <ol>
          {this.state.allTask
            ? this.props.state.userDetails.map((item, index) => {
                ...
                return (
                  <li key={index}>
                    ...
                    <p>
                      <strong>
                        {curDate > item.date ? "Due Date is Passed" : ""}{" "}
                      </strong>
                    </p>
                  </li>
                );
